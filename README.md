...
Palestras com várias apresentações mostram a tela de seleção. No player, os controles, as setas e as teclas `F` e `Escape` fazem a navegação. Ao chegar ao fim de uma apresentação HTML, a próxima apresentação é aberta automaticamente.
...
Os slides ocupam a área inteira, têm transição suave e respondem a `→`, `←` e clique nas metades direita/esquerda. Ao pressionar `→` no último slide, o motor envia `{ type: "presentation-finished" }` ao player com `window.parent.postMessage`. No primeiro slide, `←` envia `presentation-previous`.
